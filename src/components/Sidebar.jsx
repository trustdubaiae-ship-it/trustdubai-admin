...
export default function Sidebar({ page, setPage, session, adminData, canAccess, theme, setTheme }) {
  ...
  const sidebarMuted   = '#374151'
  const sidebarSection = '#1f2937'
  const activeBg       = 'rgba(74,222,128,0.08)'
  const activeColor    = '#4ade80'
  const hoverBg        = 'rgba(255,255,255,0.03)'

  return (
    <div style={{ width:210, background:'#0d1117', position:'fixed', top:0, left:0, height:'100vh', display:'flex', flexDirection:'column', zIndex:100, borderRight:'0.5px solid rgba(255,255,255,0.07)' }}>

      {/* Logo */}
      <div style={{ padding:'16px 14px 14px', borderBottom:'0.5px solid rgba(255,255,255,0.07)', display:'flex', alignItems:'center', gap:10 }}>
        ...
        <div>
          <div style={{ fontSize:14, fontWeight:700, color:'#f0fdf4', letterSpacing:'-0.2px', fontFamily:'DM Sans, sans-serif' }}>
            TRUST<span style={{ color:'#4ade80' }}>DUBAI</span>
          </div>
          <div style={{ fontSize:8.5, color:'#1d9e75', marginTop:1, fontFamily:'DM Sans, sans-serif' }}>Super Admin Panel</div>
        </div>
      </div>

      {/* Theme Toggle */}
      <div style={{ padding:'8px 14px', borderBottom:'0.5px solid rgba(255,255,255,0.07)', display:'flex', gap:4 }}>
        <button
          onClick={() => setTheme('dark')}
          style={{ flex:1, padding:'5px 0', borderRadius:6, border:'none', cursor:'pointer', fontSize:10, fontWeight:600, fontFamily:'DM Sans, sans-serif', background:isDark?'rgba(74,222,128,0.15)':'rgba(255,255,255,0.04)', color:isDark?'#4ade80':'#374151', display:'flex', alignItems:'center', justifyContent:'center', gap:4 }}>
          <i className="ti ti-moon" style={{ fontSize:11 }}/> Dark
        </button>
        <button
          onClick={() => setTheme('light')}
          style={{ flex:1, padding:'5px 0', borderRadius:6, border:'none', cursor:'pointer', fontSize:10, fontWeight:600, fontFamily:'DM Sans, sans-serif', background:!isDark?'rgba(74,222,128,0.15)':'rgba(255,255,255,0.04)', color:!isDark?'#4ade80':'#374151', display:'flex', alignItems:'center', justifyContent:'center', gap:4 }}>
          <i className="ti ti-sun" style={{ fontSize:11 }}/> Light
        </button>
      </div>
      ...
      <nav style={{ flex:1, padding:'6px 0', overflowY:'auto' }}>
        {MENU.map((item, i) => {
          if (item.section) return (
            <div key={i} style={{ fontSize:7.5, fontWeight:700, color:sidebarSection, letterSpacing:'0.08em', padding:'10px 16px 3px', textTransform:'uppercase', fontFamily:'DM Sans, sans-serif' }}>
              {item.section}
            </div>
          )
          ...
          return (
            <div key={i} onClick={() => setPage(item.id)}
              style={{ display:'flex', alignItems:'center', gap:8, padding:'7px 16px', cursor:'pointer', background:isActive?activeBg:'transparent', borderRight:isActive?`2px solid ${activeColor}`:'2px solid transparent', transition:'all 0.15s' }}
              onMouseEnter={e=>{ if(!isActive) e.currentTarget.style.background=hoverBg }}
              onMouseLeave={e=>{ if(!isActive) e.currentTarget.style.background='transparent' }}
            >
              <i className={`ti ${item.icon}`} style={{ fontSize:13, color:isActive?activeColor:sidebarMuted, flexShrink:0 }}/>
              <span style={{ fontSize:11, fontWeight:isActive?600:400, color:isActive?activeColor:'#6b7280', flex:1, fontFamily:'DM Sans, sans-serif' }}>{item.label}</span>
              {item.badge && (
                <span style={{ background:item.badgeColor+'22', color:item.badgeColor, fontSize:8, fontWeight:700, padding:'1px 5px', borderRadius:99, fontFamily:'DM Sans, sans-serif' }}>{item.badge}</span>
              )}
            </div>
          )
        })}
      ...
      {/* User Footer */}
      <div style={{ padding:'10px 14px', borderTop:'0.5px solid rgba(255,255,255,0.07)', display:'flex', alignItems:'center', gap:8 }}>
        <div style={{ width:28, height:28, borderRadius:'50%', background:'linear-gradient(135deg,#0f6e56,#1d9e75)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:10, fontWeight:700, color:'#fff', flexShrink:0, fontFamily:'DM Sans, sans-serif' }}>
          {(adminData?.full_name||session?.user?.email||'A')[0].toUpperCase()}
        </div>
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{ fontSize:10, fontWeight:600, color:'#f0fdf4', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', fontFamily:'DM Sans, sans-serif' }}>
            {adminData?.full_name||'Nadeem Ali'}
          </div>
          <div style={{ fontSize:8, color:'#1d9e75', fontFamily:'DM Sans, sans-serif' }}>
            {isSuperAdmin ? 'Super Admin' : adminData?.role||'Staff'}
          </div>
        </div>
        <button onClick={() => supabase.auth.signOut()}
          style={{ width:24, height:24, borderRadius:6, background:'rgba(248,113,113,0.1)', border:'0.5px solid rgba(248,113,113,0.2)', color:'#f87171', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>

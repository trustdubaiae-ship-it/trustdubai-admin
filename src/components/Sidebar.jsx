...
export default function Sidebar({ page, setPage, session, adminData, canAccess, theme, setTheme }) {
  ...
  const MENU = [
    ...
    { id: 'reports',           icon: 'ti-robot',            label: 'AI Moderation',     show: isSuperAdmin },
    ...
    { id: 'plans',             icon: 'ti-award',            label: 'Badges',            show: canAccess('manage_plans') },
    { id: 'notifications',     icon: 'ti-bell',             label: 'Notifications',     show: true },
    { id: 'leads',             icon: 'ti-mail',             label: 'Email Templates',   show: true },
    { section: 'SYSTEM' },
    { id: 'team',              icon: 'ti-settings',         label: 'Settings',          show: isSuperAdmin },
    { id: 'team',              icon: 'ti-lock',             label: 'Roles & Permissions', show: isSuperAdmin },
    { id: 'bulk',              icon: 'ti-notes',            label: 'Logs',              show: canAccess('bulk_upload') },
  ...
  return (
    <div style={{ width: 210, background: '#0d1117', position: 'fixed', top: 0, left: 0, height: '100vh', display: 'flex', flexDirection: 'column', zIndex: 100, borderRight: '0.5px solid rgba(255,255,255,0.07)' }}>
      ...
      {/* Logo */}
      <div style={{ padding: '16px 16px 14px', borderBottom: '0.5px solid rgba(255,255,255,0.07)', display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 8.5, color: '#1d9e75', marginTop: 1 }}>Admin Panel</div>
        </div>
      </div>
      ...
      <nav style={{ flex: 1, padding: '6px 0', overflowY: 'auto' }}>
        {MENU.map((item, i) => {
          if (item.section) return (
            <div key={i} style={{ fontSize: 7.5, fontWeight: 700, color: '#1f2937', letterSpacing: '0.08em', padding: '10px 16px 3px', textTransform: 'uppercase' }}>
              {item.section}
            </div>
          )
          ...
          return (
            <div key={i} onClick={() => setPage(item.id)}
              style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 16px', cursor: 'pointer', background: isActive ? 'rgba(74,222,128,0.08)' : 'transparent', borderRight: isActive ? '2px solid #4ade80' : '2px solid transparent', transition: 'all 0.15s', margin: '1px 0' }}
              onMouseEnter={e => { if (!isActive) e.currentTarget.style.background = 'rgba(255,255,255,0.03)' }}
              onMouseLeave={e => { if (!isActive) e.currentTarget.style.background = 'transparent' }}
            >
              <i className={`ti ${item.icon}`} style={{ fontSize: 13, color: isActive ? '#4ade80' : '#374151', flexShrink: 0 }} />
              <span style={{ fontSize: 11, fontWeight: isActive ? 600 : 400, color: isActive ? '#4ade80' : '#6b7280', flex: 1 }}>{item.label}</span>
              {item.badge && (
                <span style={{ background: item.badgeColor + '22', color: item.badgeColor, fontSize: 8, fontWeight: 700, padding: '1px 5px', borderRadius: 99 }}>{item.badge}</span>
              )}
            </div>
          )
        })}
      ...
      <div style={{ padding: '10px 14px', borderTop: '0.5px solid rgba(255,255,255,0.07)', display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 8, color: '#1d9e75' }}>
            {adminData?.role === 'superadmin' || adminData?.role === 'super_admin' ? 'Platform Lead' : adminData?.role || 'Staff'}
          </div>
        </div>

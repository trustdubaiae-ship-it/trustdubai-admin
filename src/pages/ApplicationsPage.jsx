            ...
            <div key={app.id} style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 14 }}>
                ...
                <div style={{ fontSize: 11, color: '#9ca3af' }}>
                  {new Date(app.applied_at).toLocaleDateString('en-AE')}
                </div>
              </div>

              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 12 }}>
                {[['👤 Owner', app.owner_name], ['📞 Phone', app.phone], ['✉️ Email', app.email], ['💬 WhatsApp', app.whatsapp], ['🌐 Website', app.website], ['📣 Source', app.how_heard]].filter(([,v]) => v).map(([l, v]) => (
                ...
                rejectingId === app.id ? (
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    <button onClick={() => approve(app)} disabled={processing}
                      style={{ padding: '9px 20px', background: '#10b981', color: 'white', border: 'none', borderRadius: 8, fontWeight: 600, fontSize: 13, cursor: 'pointer' }}>
                      ✅ Approve
                    </button>

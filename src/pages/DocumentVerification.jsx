...
export default function DocumentVerification({ theme, adminData }) {
  ...
  // group docs by company (only companies that uploaded at least 1)
  ...
  const filtered = useMemo(() => {
    ...
    return list.sort((a, b) => b.pending - a.pending || a.pct - b.pct)
  }, [grouped, filter, planFilter, search])

  const stats = useMemo(() => {
    const companiesCount = grouped.length
    const pendingCount = grouped.filter(g => g.pending > 0).length
    const verifiedCount = grouped.filter(g => g.pending === 0 && g.verified > 0).length
    const avg = grouped.length ? Math.round(grouped.reduce((s, g) => s + g.pct, 0) / grouped.length) : 0
    return { companiesCount, pendingCount, verifiedCount, avg }
  }, [grouped])

  function pctColor(p) { return p >= 75 ? C.green : p >= 40 ? '#fbbf24' : '#f87171' }

  const StatCard = ({ label, val, color }) => (
    <div style={{ flex: 1, background: C.card, border: `0.5px solid ${C.border}`, borderRadius: 9, padding: '9px 12px' }}>
      <div style={{ fontSize: 8.5, color: C.muted, textTransform: 'uppercase', letterSpacing: '.05em' }}>{label}</div>
      <div style={{ fontSize: 19, fontWeight: 800, color: color || C.text, marginTop: 2 }}>{val}</div>
    </div>
  )
  ...
  const CompanyCard = ({ g }) => {
    ...
    return (
      <div style={{ background: C.card, border: `0.5px solid ${C.border}`, borderRadius: 11, padding: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 9 }}>
          <div style={{ width: 32, height: 32, borderRadius: 8, background: g.comp.logo_url ? 'transparent' : (PLAN_GRAD[g.comp.plan] || PLAN_GRAD.free), display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: 12, color: '#fff', flexShrink: 0, overflow: 'hidden' }}>
            {g.comp.logo_url ? <img src={g.comp.logo_url} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : (g.comp.name?.[0]?.toUpperCase() || '?')}
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: C.text, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{g.comp.name || '—'}</div>
        ...
        <span style={{ display: 'inline-block', background: fully ? 'rgba(74,222,128,0.18)' : 'rgba(251,191,36,0.18)', color: fully ? '#4ade80' : '#fbbf24', fontSize: 9, fontWeight: 700, padding: '2px 7px', borderRadius: 99, marginBottom: 9 }}>
          {fully ? 'Verified' : `${g.pending} pending`}
        </span>
        <button onClick={() => setActiveCompany(g)} style={{ width: '100%', padding: 6, borderRadius: 7, border: fully ? `0.5px solid ${C.border}` : 'none', background: fully ? 'transparent' : C.green, color: fully ? C.sub : '#fff', fontSize: 11, fontWeight: 700, cursor: 'pointer' }}>Review →</button>
      </div>
    )
  }
  ...
  const CompanyRow = ({ g }) => {
    const fully = g.pending === 0 && g.verified > 0
    return (
      <div style={{ background: C.card, border: `0.5px solid ${C.border}`, borderRadius: 11, padding: '11px 14px', display: 'flex', alignItems: 'center', gap: 12 }}>
  ...
  return (
    <div>
      <div style={{ marginBottom: 16 }}>
        ...
        <p style={{ fontSize: 13, color: C.sub, margin: '4px 0 0' }}>Review documents by company. Approve only after checking every detail against the document.</p>
      </div>

      <div style={{ display: 'flex', gap: 7, marginBottom: 12 }}>
        <StatCard label="Companies" val={stats.companiesCount} />
        <StatCard label="Pending" val={stats.pendingCount} color="#fbbf24" />
        <StatCard label="Verified" val={stats.verifiedCount} color="#4ade80" />
        <StatCard label="Avg Score" val={`${stats.avg}%`} />
      </div>

      <div style={{ display: 'flex', gap: 6, marginBottom: 14, flexWrap: 'wrap', alignItems: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 5, background: C.card, border: `0.5px solid ${C.border}`, borderRadius: 8, padding: '6px 10px', flex: 1, minWidth: 130 }}>
        ...
        {[['pending','Has Pending'],['verified','Verified'],['all','All']].map(([k, l]) => (
          <button key={k} onClick={() => setFilter(k)} style={{ padding: '6px 11px', borderRadius: 8, border: `0.5px solid ${filter===k ? C.green : C.border}`, background: filter===k ? 'rgba(34,197,94,0.12)' : C.card, color: filter===k ? '#4ade80' : C.sub, fontSize: 10.5, fontWeight: 600, cursor: 'pointer' }}>{l}</button>
        ))}
      ...
      {activeCompany && (
        <CompanyDocsModal
          ...
          onDone={() => { setActiveCompany(null); load() }}
        />
      )}

      <style>{`@media (max-width:1100px){.docv-grid{grid-template-columns:repeat(2,1fr)!important}}@media (max-width:560px){.docv-grid{grid-template-columns:1fr!important}}`}</style>
    </div>
  )
}

/* ---------- Company docs list modal ---------- */
function CompanyDocsModal({ group, docMeta, adminData, C, isDark, onClose, onDone }) {
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
            ...
            return (
              <div key={d.id} style={{ background: C.bg2, border: `0.5px solid ${C.border}`, borderRadius: 10, padding: '11px 13px', display: 'flex', alignItems: 'center', gap: 11 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 11, color: C.sub }}>No. {d.doc_number || '—'}</div>
                </div>
      ...
      {reviewDoc && (
        <ReviewModal
          row={reviewDoc}
          meta={metaMap[reviewDoc.doc_key]}
          company={group.comp}
          adminData={adminData}
          C={C} isDark={isDark}
          onClose={() => setReviewDoc(null)}
          onDone={() => { setReviewDoc(null); onDone() }}
        />
      )}
    </div>
  )

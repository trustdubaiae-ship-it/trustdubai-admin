...
const fComp   = (l) => pick(l, ['company_id', 'companyId', 'business_id'])
const cName   = (c) => pick(c, ['name', 'company_name', 'business_name', 'title']) || 'Unnamed'
const cPlan   = (c) => (pick(c, ['plan', 'plan_tier', 'tier', 'plan_name', 'subscription']) || 'free')
...
const normStatus = (raw) => {
  const s = norm(raw)
  if (!s) return 'new'
  if (/(^|[^a-z])new|fresh|open\b/.test(s)) return 'new'
...
const normSource = (raw) => {
  ...
  if (/form|web|site|landing/.test(s)) return 'Form'
...
const normTemp = (raw) => {
  const s = norm(raw)
  if (/hot|high/.test(s)) return 'hot'
  if (/warm|med/.test(s)) return 'warm'
  if (/cold|low/.test(s)) return 'cold'
  return ''
}
...
const aiScore = (lead) => {
  ...
  s += t === 'hot' ? 40 : t === 'warm' ? 25 : t === 'cold' ? 10 : 15
  const created = fCreated(lead)
  const days = created ? daysBetween(Date.now(), new Date(created).getTime()) : 999
  s += days <= 3 ? 25 : days <= 7 ? 20 : days <= 14 ? 15 : days <= 30 ? 10 : 5
  const src = normSource(fSource(lead))
  s += (src === 'Meta' || src === 'Form') ? 20 : src === 'WhatsApp' ? 15 : src === 'Manual' ? 10 : 12
  const b = parseBudget(fBudget(lead))
  s += b >= 100000 ? 15 : b >= 50000 ? 12 : b >= 20000 ? 8 : b > 0 ? 5 : 6
  ...
  return (
    <div ref={ref} style={{ width: '100%' }}>
      <svg width={w} height={height} className="re-line">
        <defs>
          <linearGradient id="reArea" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor="#6366f1" stopOpacity="0.35" />
            <stop offset="100%" stopColor="#6366f1" stopOpacity="0" />
          </linearGradient>
        </defs>
        {[0, 0.5, 1].map((g, i) => (
          <line key={i} x1={pad} x2={w - pad} y1={pad / 2 + innerH - g * innerH} y2={pad / 2 + innerH - g * innerH}
            stroke="var(--re-track)" strokeWidth="1" strokeDasharray="3 4" />
        ))}
        <path d={area} fill="url(#reArea)" />
        <path d={path} fill="none" stroke="#6366f1" strokeWidth="2.5" strokeLinejoin="round" strokeLinecap="round" />
        {last && <circle cx={x(points.length - 1)} cy={y(last.v)} r="5" fill="#6366f1" stroke="#fff" strokeWidth="2" />}
      </svg>
    </div>
  )
}
        ...
        return (
          <div className="re-funnel-row" key={i}>
            <div className="re-funnel-bar-wrap">
              <div className="re-funnel-bar" style={{ width: `${Math.max(pct, 8)}%`, background: s.color }} />
            </div>
...
function Gauge({ value }) {
  ...
  return (
    <svg width="180" height="110" viewBox="0 0 180 110" className="re-gauge">
      <defs>
        <linearGradient id="reGauge" x1="0" x2="1">
          <stop offset="0%" stopColor="#ef4444" /><stop offset="50%" stopColor="#fbbf24" /><stop offset="100%" stopColor="#22c55e" />
        </linearGradient>
      </defs>
      <path d={`M ${sx} ${sy} A ${r} ${r} 0 0 1 ${ex} ${ey}`} fill="none" stroke="var(--re-track)" strokeWidth="14" strokeLinecap="round" />
      <path d={`M ${sx} ${sy} A ${r} ${r} 0 ${value > 50 ? 1 : 0} 1 ${vx} ${vy}`} fill="none" stroke="url(#reGauge)" strokeWidth="14" strokeLinecap="round" />
      <text x="90" y="78" textAnchor="middle" className="re-gauge-num" style={{ fill: color }}>{value}</text>
      <text x="90" y="98" textAnchor="middle" className="re-gauge-sub">Average Score</text>
    </svg>
  )
}
...
function HBars({ rows }) {
  ...
  return (
    <div className="re-hbars">
      {rows.map((r, i) => (
        <div className="re-hbar-row" key={i}>
          <span className="re-hbar-rank">{i + 1}</span>
          <span className="re-hbar-name" title={r.label}>{r.label}</span>
          <div className="re-hbar-track"><div className="re-hbar-fill" style={{ width: `${(r.value / max) * 100}%`, background: r.color || '#6366f1' }} /></div>
          <span className="re-hbar-val">{r.value}</span>
        </div>
      ))}
...
/* ============================== MAIN ==================================== */
export default function RevenueEngine({ setPage, theme = 'dark', adminData }) {
  ...
  const m = useMemo(() => {
    ...
    const last30 = leads.filter(l => fCreated(l) && new Date(fCreated(l)).getTime() >= d30).length
    const prev30 = leads.filter(l => {
      const t = fCreated(l) ? new Date(fCreated(l)).getTime() : 0
      return t >= d60 && t < d30
    }).length
    ...
    const won30 = leads.filter(l => normStatus(fStatus(l)) === 'won' && fCreated(l) && new Date(fCreated(l)).getTime() >= d30).length
    const wonPrev = leads.filter(l => {
      const t = fCreated(l) ? new Date(fCreated(l)).getTime() : 0
    ...
    leads.forEach(l => {
      const f = fFollow(l)
      if (!f) return
      const st = normStatus(fStatus(l))
      if (st === 'won' || st === 'lost') return
      const fd = startOfDay(new Date(f)).getTime()
      const td = today.getTime()
      if (fd === td) { dueToday++; followDue++ }
    ...
    const cNameById = {}
    companies.forEach(c => { cNameById[c.id] = cName(c) })
    ...
    for (let wk = 4; wk >= 0; wk--) {
      ...
      for (let dy = 0; dy < 7; dy++) {
        const base = startOfDay(now - (wk * 7 + (6 - dy)) * 86400000).getTime()
        const next = base + 86400000
        const cnt = leads.filter(l => {
          const f = fFollow(l); if (!f) return false
          const t = startOfDay(new Date(f)).getTime()
    ...
    if (hot > 0) insights.push(`${hot} hot lead${hot > 1 ? 's' : ''} need attention now — prioritise these today.`)
    if (insights.length === 0) insights.push('No leads yet. Insights will appear as soon as data starts flowing in.')

    return {
      total, last30, totalChange: change(last30, prev30),
      conversion, convChange: change(won30, wonPrev),
      hot, hotChange: change(hot30, hotPrev),
      followDue, dueToday, overdue,
    ...
    <div className="re-root" data-theme={isDark ? 'dark' : 'light'}>
      ...
      <div className="re-header">
        <div>
          ...
          <p className="re-subtitle">Welcome back, {adminName}! 👋 Here's how your sales pipeline is performing.</p>
        </div>
        <div className="re-header-right">
          <div className="re-clock">
            <span className="re-clock-time">{clock.toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' })}</span>
            <span className="re-clock-date">{clock.toLocaleDateString('en-GB', { weekday: 'short', day: 'numeric', month: 'short' })}</span>
          </div>
      ...
      {loading && leads.length === 0 ? (
        ...
        <>
          ...
          <div className="re-kpis">
            ...
            <KPI icon={Ic.fire}   tint="#f97316" label="Hot Leads"            value={m.hot}               change={m.hotChange} />
            <KPI icon={Ic.clock}  tint="#06b6d4" label="Follow-ups Due"       value={m.followDue}         change={null} />
            <KPI icon={Ic.send}   tint="#22c55e" label="Distributed Leads"    value={m.distCount}         change={null} />
          ...
          <div className="re-grid re-grid-4">
            ...
            <Card title="Leads by Source">
              <div className="re-donut-wrap">
                <Donut data={m.sources} total={m.total} size={140} />
                <Legend data={m.sources} />
              </div>
            </Card>
            <Card title="Leads Trend (30 days)">
              <LineTrend points={m.trend} />
            </Card>
            <Card title="Follow-ups">
              <div className="re-follow">
                <button className="re-follow-box re-amber" onClick={() => setPage && setPage('leads')}>
                  <span className="re-follow-num">{m.dueToday}</span>
                  <span className="re-follow-label">Due Today</span>
                </button>
                <button className="re-follow-box re-red" onClick={() => setPage && setPage('leads')}>
                  <span className="re-follow-num">{m.overdue}</span>
                  <span className="re-follow-label">Overdue</span>
                </button>
              </div>
              <div className="re-insights">
                <div className="re-insights-head">✨ AI Insights</div>
          ...
          <div className="re-grid re-grid-3">
            ...
            <Card title="Leads by Category / Project Type">
              <div className="re-donut-wrap">
                <Donut data={m.types} total={m.total} centerLabel={m.total} centerSub="Total" size={150} />
                <Legend data={m.types} />
              </div>
            </Card>
            <Card title="Follow-ups Heatmap">
              <div className="re-heat">
                <div className="re-heat-days">
                  {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map(d => <span key={d}>{d}</span>)}
                </div>
                {m.heat.map((row, wi) => (
                  <div className="re-heat-row" key={wi}>
                    {row.map((c, di) => <span key={di} className="re-heat-cell" style={{ background: heatColor(c) }} title={`${c} follow-ups`} />)}
                  </div>
                ))}
            ...
            <div className="re-plan-grid">
              ...
              {m.planRev.map(p => (
                <div className="re-plan-card" key={p.key}>
                  <div className="re-plan-card-top">
                    <span className="re-plan-dot" style={{ background: p.color }} />
                    <span className="re-plan-name">{p.label}</span>
                  </div>
                  <span className="re-plan-rev">AED {fmtAED(p.revenue)}</span>
                  <span className="re-plan-count">{p.count} companies</span>
                </div>
              ))}
            </div>
          </Card>
          ...
          <div className="re-grid re-grid-4">
            <Card title="AI Lead Score Distribution">
              <div className="re-score">
                ...
                <Legend data={m.scoreBuckets} />
              </div>
            </Card>
            <Card title="Lead Status">
              <div className="re-donut-wrap">
                <Donut data={m.statusDonut} total={m.total} centerLabel={m.total} centerSub="Total" size={140} />
                <Legend data={m.statusDonut} />
              </div>
            </Card>
            <Card title="Conversion Rate by Source">
              <VBars rows={m.convBySrc} />
            </Card>
            <Card title="Geographic Leads (Top Areas)">
              <div className="re-collecting">
                <div className="re-collecting-icon">📍</div>
                <p className="re-collecting-title">Collecting data…</p>
                <p className="re-collecting-sub">Add an <b>Area / Location</b> field to your lead form to unlock area-wise insights here.</p>
              </div>
            </Card>
          </div>
            ...
            <div className="re-activity">
              ...
              {m.activity.map((a, i) => (
                <div className="re-activity-row" key={i}>
                  <span className="re-activity-dot" />
                  <span className="re-activity-text">{a.text}</span>
                  <span className="re-activity-time">{a.time}</span>
                </div>
              ))}
            </div>
          </Card>
  ...
  return (
    <div className="re-kpi">
      <div className="re-kpi-top">
        ...
        <span className="re-kpi-label">{label}{sub && <em className="re-kpi-sub"> {sub}</em>}</span>
      </div>
      ...
      {change != null
        ...
        : <div className="re-kpi-change muted">— current</div>}
    </div>
  )
}
...
.re-root{
  ...
  width:100%; box-sizing:border-box; padding:clamp(12px,2vw,24px);
}
.re-root *{box-sizing:border-box;}
.re-root[data-theme="dark"]{
  --re-bg:#0d1117; --re-card:#161b22; --re-card2:#1c232c;
...
.re-root[data-theme="light"]{
  --re-bg:#f5f7fb; --re-card:#ffffff; --re-card2:#f8fafc;
...
.re-header-right{display:flex;align-items:center;gap:12px;flex-wrap:wrap;}
.re-clock{display:flex;flex-direction:column;align-items:flex-end;line-height:1.1;}
.re-clock-time{font-size:18px;font-weight:700;}
.re-clock-date{font-size:11px;color:var(--re-muted);}
.re-refresh{display:flex;align-items:center;gap:7px;background:var(--re-card);border:1px solid var(--re-border);color:var(--re-text);
...
.re-kpi-sub{font-style:normal;font-size:10px;opacity:.7;display:block;}
...
.re-hbar-name{width:110px;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;flex-shrink:0;}
...
.re-plan-total-num{font-size:28px;font-weight:800;}
.re-plan-total-sub{font-size:11px;opacity:.8;}
.re-plan-card{background:var(--re-card2);border:1px solid var(--re-border);border-radius:14px;padding:16px;display:flex;flex-direction:column;gap:6px;}
.re-plan-card-top{display:flex;align-items:center;gap:8px;}
.re-plan-dot{width:10px;height:10px;border-radius:50%;}
...
@media(max-width:560px){
  ...
  .re-header-right{width:100%;justify-content:space-between;}
  .re-hbar-name{width:80px;}
}
@media(max-width:380px){
  .re-kpis{grid-template-columns:1fr;}
  .re-plan-grid{grid-template-columns:1fr;}
}
`
